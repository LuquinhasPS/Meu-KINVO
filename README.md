...
- `data/`: Diretório contendo os dados da aplicação.
  ...
  - `all_tickers.json`: Lista completa de ativos disponíveis (gerada automaticamente).
  - `historico_portfolio.csv`: Histórico do valor total da carteira para o gráfico de evolução.
- `scripts/`: Diretório com scripts utilitários (opcional) para manutenção.
  - `update_tickers.py`: Atualiza a lista de ativos disponíveis.
  - `backfill_historico.py`: Popula o histórico de valor da carteira retroativamente.
- `requirements.txt`: Lista de todas as bibliotecas Python necessárias.
...
![Detalhes dos Ativos](screenshots/painel-detalhes-ativos.png)

### Análise Gráfica
Gráficos interativos permitem visualizar a alocação da carteira (por ativo e tipo).

![Análise Gráfica](screenshots/analise-grafica-carteira.png)

### Análise de Aportes
Evolução dos aportes realizados ao longo do tempo e lista detalhada de transações.

![Análise de Aportes](screenshots/analise-aportes.png)

### Formulário de Transações
Adicione novas compras facilmente através da barra lateral.

![Formulário de Nova Compra](screenshots/formulario-nova-compra.png)